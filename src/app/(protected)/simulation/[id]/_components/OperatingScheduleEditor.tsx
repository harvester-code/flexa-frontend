...
import { Clock, Plane, Users, MapPin, Star, Trash2, Settings } from 'lucide-react';
...
export default function OperatingScheduleEditor({ processFlow }: OperatingScheduleEditorProps) {
  ...
  const handleCellMouseDown = useCallback((cellId: string, rowIndex: number, colIndex: number, e: React.MouseEvent) => {
    ...
    setSelectedCells(prev => {
      setDragState(createDragState('cell', { row: rowIndex, col: colIndex }, isAdditive, isAdditive ? new Set(prev) : null));
  ...
  const handleColumnMouseDown = useCallback((colIndex: number, e: React.MouseEvent) => {
    ...
    setSelectedCells(prev => {
      setDragState(createDragState('column', { row: 0, col: colIndex }, isAdditive, isAdditive ? new Set(prev) : null));
          ...
          <div className="mb-4 p-4 bg-slate-50 rounded-lg border">
            <div className="flex items-center gap-4">
              ...
              <div className="text-xs text-slate-500">
                Current: {currentFacilityCount} facilities
              </div>
            </div>
          </div>
        )}
