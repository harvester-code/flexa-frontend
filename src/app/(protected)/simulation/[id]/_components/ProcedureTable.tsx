        ...
        ? {
            ...
            values: Array(header.length)
              ...
              .map((val, cidx) => {
                ...
                return !nextCheckedAll ? 0 : cidx == header.length - 1 ? String(remainPer) : String(divPer);
              }),
          }
        : row
  ...
  const handleChangeCheck = (rowIndex: number, colIndex: number) => {
    const newData = data.map((row, ridx) =>
      rowIndex == ridx
        ? {
            ...row,
            values: [...row.values].map((val, cidx) => (colIndex == cidx ? (Number(val) > 0 ? 0 : 10) : val)),
          }
        : row
    ) as ProcedureTableRow[];
    let checkedCount = 0;
      ...
      rowIndex == ridx
        ? {
            ...
            values: [...row.values].map((val, cidx) =>
              colIndex == cidx ? (type == 'probability' ? Math.max(Number(value), 0).toString() : value) : val
            ),
          }
        : row
    ) as ProcedureTableRow[];
                ...
                <div className="flex items-center justify-center gap-[10px]">
                  {type == 'checkbox' ? (
                    ...
                    <Input
                      ...
                      placeholder=""
                      value={value}
                      className="!border-none bg-transparent text-center !text-default-700"
