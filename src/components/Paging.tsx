  ...
  return (
    <div className="paging">
      <button className="arrow-button">
        <FontAwesomeIcon className="icon-14" icon={faAnglesLeft} onClick={() => { if(onChangePage) onChangePage(Math.max(currentPage - visiblePageAmount, 1)); }} />
      </button>
      <button className="arrow-button" onClick={() => { if(onChangePage) onChangePage(Math.max(currentPage - 1, 1)); }}>
      ...
      {Array.from({ length: Math.min(visiblePageAmount, totalPage - startPage + 1) }, (_, index) => (
        <button
          ...
          onClick={() => { if(onChangePage) onChangePage(startPage + index); }}
        >
          {startPage + index}
        </button>
      ))}
      <button className="arrow-button" onClick={() => { if(onChangePage) onChangePage(Math.min(currentPage + 1, totalPage)); }}>
        <FontAwesomeIcon className="icon-14" icon={faAngleRight} />
      </button>
      <button className="arrow-button" onClick={() => { if(onChangePage) onChangePage(Math.min(currentPage + visiblePageAmount, totalPage)); }}>
