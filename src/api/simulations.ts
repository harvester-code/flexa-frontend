...
export const useScenarioList = (group_id?: number) => {
  const response = useQuery({
    ...
    queryFn: () => getScenarioList(group_id).then<IScenarioData[]>((response) => {
      const masterScenarios = {};
      for(const rowCur of response?.data?.master_scenario || []) if(rowCur?.id) masterScenarios[rowCur.id] = rowCur;
      return [...(response?.data?.master_scenario.filter((val => val != null)) || []), ...(response?.data?.user_scenario?.filter((val) => val.id in masterScenarios == false) || [])];
    }),
...
export const modifyScenario = (params: {
  id: string;
  simulation_name?: string;
  memo?: string;
...
export const getScenarioMetadata = (params: {
  simulation_id: string;
}) => axios.get<IScenarioMetadataResponse>(`/api/v1/simulations/scenario/metadata?${new URLSearchParams(params).toString()}`);

export const setScenarioMetadata = (params: Partial<Omit<IScenarioMetadata, 'id'>>) => axios.put(`/api/v1/simulations/scenario/metadata`, params);
...
export const getFlightSchedule = (simulation_id: string, params: {
  first_load: boolean;
  airport: string;
  date: string;
  condition: Array<{
...
}) => axios.post<IFlightScheduleResponse>(`/api/v1/simulations/flight-schedule?simulation_id=${simulation_id}`, params);
